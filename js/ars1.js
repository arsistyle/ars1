...
var ars1 =  {
	...
	accordeon : function (selector, activo) {
		'use strict';
		var _selector = document.querySelector(selector);
		if (_selector && _selector.className) {
			var _this = this;
			var _activo = false;
			if (typeof activo === 'number') _activo = activo;
			_this.opciones = {
				activo: _activo,
				header: selector + '__header',
				body: selector + '__body'
			};

			var _opciones = _this.opciones;
			var _mover;
			$(_opciones.header).each(function (i, el) {
				var _this = $(this);
				_this.next().hide().attr('id', 'accordeon-body-' + i);
				var _id_next = _this.next().attr('id');
				if (_this.hasClass('active')) {
					_this.next().show().addClass('active');
				} else if (typeof _opciones.activo === 'number') {
					$(_opciones.header).eq(_opciones.activo).next().show().addClass('active');
				}
				_this.click(function () {
					$(_opciones.header).not(_this).removeClass('active');
					$(_opciones.body).not(_this.next()).slideUp(600).removeClass('active');
					if (!_this.hasClass('active')) {
			...
			console.error('Accordeon: Se debe crear y definir un selector con clase');
		}
	},
	lightbox :  {
		creado: false,
		...
		abrir: function (el) {
			"use strict";
			ars1.lightbox.cerrado = false;

			var id = $(el).attr('href');

			if (!ars1.lightbox.cerrado) {
				$('.lightbox--activo').removeClass('lightbox--activo').addClass('lightbox--hidden');
				ars1.lightbox.generar();
				$(ars1.lightbox.selector).find(id).removeClass('lightbox--hidden').addClass('lightbox--activo');
				$(ars1.lightbox.selector).fadeIn('fast');

				setTimeout(function () { }, 200);

				ars1.lightbox.creado = true;
			}
		},
		cerrar: function () {
			"use strict";
			if (ars1.lightbox.creado) {
				if (!ars1.lightbox.modal) {
					$('html').css('overflow', '');
					$(ars1.lightbox.selector).fadeOut('fast');
					setTimeout(function () {
						$(ars1.lightbox.selector).find('.lightbox--activo').removeClass('lightbox--activo').addClass('lightbox--hidden');
					}, 500);
				}
				ars1.lightbox.creado = false;
				ars1.lightbox.cerrado = true;
			}
		},
		init: function () {
			$('.abrir-lightbox').click(function () {
				...
				ars1.lightbox.abrir(this);
				return false;
			});
			$('.cerrar-lightbox').click(function () {
				ars1.lightbox.cerrar();
				return false;
			});
